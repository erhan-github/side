...
class ContextCompressor:
    ...
    async def compress(
        ...
        # Weighting: Recent decisions have 3x priority.
        # Decisions older than 30 days are pruned unless they are manually pinned (future feature)
        decisions = [d for d in decisions if is_recent(d)]
        goals = [g for g in goals if is_recent(g) or g.get("priority", 0) > 5]
        learnings = [l for l in learnings if is_recent(l)]
        ...
        # Current length estimation
        def get_total_len(*args):
            return sum(len(str(a)) for a in args)

        total = get_total_len(q_str, p_str, compressed_goals, compressed_decisions, compressed_learnings, compressed_articles, recent_changes)
    ...
    def _format_list(self, items: List[Dict[str, Any]], label: str, max_items: int, recent_first: bool = False) -> str:
        ...
        if recent_first:
            try:
                working_items = sorted(items, key=lambda x: x.get("created_at", ""), reverse=True)
            except:
                working_items = items
        else:
            working_items = items
