...
class HealthEngine:
    ...
    UPGRADED: Semantic Awareness via Tree-sitter.
    """
    ...
    def _execute_rule(self, rule: DynamicRule, content: str, filepath: str) -> Optional[str]:
        ...
        try:
            ...
            # 2. Semantic Audit - Higher Precision
    ...
    def capture_decision_trace(self, rule_id: str, fix_applied: str, context: Dict) -> bool:
        ...
        from side.utils.crypto import shield as system_shield
        scrubbed_fix = system_shield.scrub(fix_applied)
        ...
        cloud_payload = [
            {
                ...
                "rationale": "System Prime Standard: No cleartext credentials in code.",
    ...
    def _load_anchor(self) -> Optional[Dict]:
        """Loads the System Reference (Intent File)."""
    ...
    def verify_gold_standard(self, file_path: str, content: str = None) -> Optional[str]:
        """Verifies a file against the System Gold Standards."""
        ...
        if actual_hash != expected_hash:
             return f"Gold Standard Violation | Hash mismatch for '{file_path}' | Rationale: This file belongs to the '{key}' Gold Standard and must match the System Hash. | FIX: Revert changes."
        
        return None

    def check_health(self, current_context: Dict = {}) -> HealthResult:
        """
        The Main Loop. Dynamically executes promoted rules.
        """
        start_time = time.perf_counter()
        violations = []
        status = HealthStatus.SECURE
        
        anchor = self._load_anchor()
        if not anchor:
            return HealthResult(status=HealthStatus.DRIFT, latency_ms=0.0, violations=["Reference Not Found"], context={})
        ...
        for invariant in invariants:
            ...
            if "port ==" in rule:
                ...
                if active_port != target_port:
                    ...
                    if level == "IMMUTABLE": status = HealthStatus.VIOLATION
            
            if "branch ==" in rule:
                target_branch = rule.split("==")[1].strip().strip('"')
                active_branch = current_context.get("BRANCH", "main")
                if active_branch != target_branch:
                    violations.append(f"Context Violation | Working on branch '{active_branch}' but System requires '{target_branch}' | Rationale: 'branch_lock' enforces alignment with the Main Branch. | FIX: Switch to branch '{target_branch}' or update the Reference.")
                    if level == "IMMUTABLE": status = HealthStatus.VIOLATION

        # 4. Handle Enforcement Mode from Anchor
        moat_health = anchor.get("moat_health", {})
        enforcement_mode = moat_health.get("enforcement_mode", "WARN")
        is_strict = enforcement_mode in ["BLOCK", "STRICT"]

        # TRUST CALIBRATION LOGIC
        if violations and status == HealthStatus.SECURE:
             status = HealthStatus.VIOLATION if is_strict else HealthStatus.DRIFT

        if status == HealthStatus.VIOLATION and not is_strict:
             status = HealthStatus.DRIFT
    ...
    def certify_repo(self) -> Dict:
        ...
        """
        print(f"🔱 [SYSTEM CERTIFICATION]: Initiating Deep Audit...")
        ...
        cert = {
            ...
            "status": "CERTIFIED" if result.status == HealthStatus.SECURE else "DENIED",
            "scores": {
                "determinism": 100 if result.status == HealthStatus.SECURE else 70,
                "privacy": 100, # Hardcoded moat
                "memory_integrity": 95
            },
            "violations": result.violations,
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "signature": "sig_sidelith_system_v1_gold"
        }
