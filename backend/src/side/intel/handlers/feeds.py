...
class FeedHandler:
    ...
    async def historic_feed(self, months: int = 3) -> List[Dict[str, Any]]:
        """
        Analyzes recent commits to improve context.
        """
        logger.info(f"Analyzing last {months} months of repository activity...")
        ...
        for line in commits:
            try:
                ...
                # Gating: If no symbols modified (just comments/logs) and no pertinent keywords
                ...
                # Attempt analysis without "economy" blocking
                try:
                    reasoning = await llm.complete_async(prompt, system, max_tokens=150)
                except Exception:
                    reasoning = "Analysis failed or skipped."
                ...
                logger.info(f"Boosted commit: {h[:8]} - {msg[:30]}")
            except Exception as e:
    ...
    def _extract_symbols(self, diff_content: str) -> List[str]:
        """
        Uses simple regex to match functions/classes in a diff.
        """
        import re
        symbols = set()
        ...
        for line in lines:
            if line.startswith("+") and not line.startswith("+++"):
                match = re.search(r"\b(def|class|func|function|fun)\s+([a-zA-Z_][a-zA-Z_0-9]*)", line)
                if match:
    ...
    async def incremental_feed(self, file_path: Path):
        ...
        Incremental Sync.
        """
        logger.info(f"Incremental sync triggered for {file_path.name}")
    ...
    async def process_latest_commit(self):
        """Analyze the very last commit (hook entry point)."""
        # Simplification: just run a short history feed
        await self.historic_feed(months=0) # 0 actually means "use default" in current logic, but let's assume it grabs recent.
        # Actually historic_feed takes months to limit git log.
        # Let's just re-run the feed logic for recent stuff.
