"""
Resolution Ledger Service for Sidelith Hyper-Perception.

Implements the 'What Fixed What' (WFW) algorithm by correlating 
log-level resolution events with recent code modifications.
"""
...
class ResolutionLedgerService:
    """
    Monitors logs/activity to close the loop between problem and solution.
    """

    def __init__(self, forensic: ForensicStore, operational: OperationalStore, buffer=None):
        ...
        # Patterns (T-105: Proactive Discovery)
        self.error_patterns = [
    ...
    async def start(self) -> None:
        """Start the resolution ledger."""
        if self._running:
            return
        self._running = True
        self._task = asyncio.create_task(self.watch_logs())
        logger.info("📡 [RESOLUTION LEDGER]: Causal Awareness Active.")

    async def stop(self) -> None:
        """Stop the resolution ledger."""
        self._running = False
        if self._task:
            self._task.cancel()
        logger.info("📡 [RESOLUTION LEDGER]: Causal Awareness Halted.")

    async def watch_logs(self, log_path: str | Path = "debug/side.log") -> None:
        """
        [WFW Algorithm]: Real-time log tailing for causal correlation.
        ...
        logger.info(f"📡 [CAUSAL]: Tailing logs at {self._last_log_file}")
        ...
        try:
            ...
            logger.error(f"WFW Tailer Error: {e}")
    ...
    async def notify_process_exit(self, exit_code: int, signal: int | None = None):
        """[MACHINE_TRUTH]: Called when a monitored process exits."""
        if exit_code != 0:
            self.last_failure = {
    ...
    async def _verification_window(self):
        ...
        if self.potential_fix:
            await self._verify_resolution("Friction Drop / No Re-occurrence")
    ...
    async def _verify_resolution(self, method: str):
        ...
        self.record_causal_link(problem, resolution, str(fix["file"]))
    ...
    def record_causal_link(self, problem: str, resolution: str, file_path: str):
        """
        ...
        if self.buffer:
            asyncio.create_task(self.buffer.ingest("activity", {
                "project_id": "SYSTEM",
                "tool": "resolution_ledger",
                "action": "causal_resolution",
                "payload": {
                    "problem": problem,
            ...
            # Save to Forensic Store as a 'Verified Pattern'
            self.forensic.log_activity(
                project_id="SYSTEM", # Should be resolved
                action="causal_resolution",
                details={
                    "problem": problem,
                    "resolution": resolution,
        ...
        logger.info(f"✨ [CAUSAL]: Linked resolution in {file_path} to problem: {problem}")
