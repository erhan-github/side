...
class MerkleTree:
    ...
    def get_project_root() -> str:
        ...
        active_ids = HistoryManager.list_active()
        if not active_ids:
            return hashlib.sha256(b"empty").hexdigest()
        
        # Collect HEAD signatures from each history
        signatures = []
        for fix_id in active_ids:
            history = HistoryManager.get(fix_id)
            if history and history.chain:
                head = history.chain[-1]
    ...
    @staticmethod
    def validate_history_inclusion(fix_id: str) -> bool:
        """
        Checks if a specific history is currently part of the active merkle set.
        """
        # In a real Merkle Tree this would be a proof, 
        # here we just check presence for now.
        return HistoryManager.get(fix_id) is not None
