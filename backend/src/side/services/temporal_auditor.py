...
class TemporalAuditorService:
    """
    [KAR-5] Temporal Forensic Chronology Service.
    Hooks into the CPython runtime to capture 'Machine Intent'.
    """
    ...
    async def start(self):
        ...
        # [KAR-6.2] Sovereign Failure Ingestion (SFI)
        self._setup_failure_hooks()
        
        logger.info("🕰️ [TEMPORAL]: Audit & Failure Hooks Registered. Listening for Intent...")
    ...
    def _audit_hook(self, event: str, args: tuple):
        ...
        """
        try:
            # Selective Filtering (Tbilisi Consensus: Ignore noise)
            relevant_events = {
            ...
            # Construct the Signal with High Fidelity (T-001)
            signal = {
                "event": event,
    ...
    def _setup_failure_hooks(self):
        ...
        # 1. Runtime Warnings (Deprecations, Smells)
        ...
        self.log_handler = SovereignLogHandler(self)
    ...
    def _calculate_momentum(self, event: str) -> float:
        """Assigns 'Strategic Weight' to events based on Tbilisi Consensus."""
        weights = {
    ...
    async def _persistence_loop(self):
        """Pass signals to the UnifiedBuffer."""
        while self._running:
            try:
                signal = await self._queue.get()
                
                # [KAR-6.12] Route to Unified Buffer
                payload = {
                ...
                logger.error(f"Temporal persistent failure: {e}")
    ...
    async def stop(self):
        ...
        logger.info("🕰️ [TEMPORAL]: Auditor & SFI Offline.")

class SovereignLogHandler(logging.Handler):
    """Bridge for ingesting logging.ERROR/WARNING into the Sovereign Store."""
    def __init__(self, auditor: TemporalAuditorService):
