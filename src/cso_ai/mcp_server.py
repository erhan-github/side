"""
CSO.ai MCP Server - "Grammarly for Strategy"
...
class CSOMCPServer:
    """MCP Server for CSO.ai forensic intelligence."""

    def __init__(self, project_root: str):
        self.project_root = Path(project_root)
        self.server = Server("cso-ai")
        
        # Initialize core components
        self.db = SimplifiedDatabase(str(self.project_root / ".cso" / "local.db"))
    ...
    async def _get_strategic_alerts(self, args: dict) -> list[TextContent]:
        ...
        # Optionally rescan
        if rescan:
            findings = self.forensic_engine.scan()
    ...
    async def _get_strategic_iq(self) -> list[TextContent]:
        ...
        output = f"**Strategic IQ: {score}/160** {grade}\n\n"
...
async def main():
    ...
    server = CSOMCPServer(project_root)
    await server.run()
